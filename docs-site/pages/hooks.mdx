...
Lightweight alternative to the standalone [`@wigtoken/agent`](/agent). One block in `~/.claude/settings.json` and every Claude Code session you run on that machine streams its messages into your wigtoken server.
